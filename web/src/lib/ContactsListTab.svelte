<script>
  ...
  async function refreshList() {
    const tabId = tabStore.findByKey("persistent-contacts-list") || tabStore.active.id;
    try {
    ...
    {#if showSearch}
      ...
      >
        {#snippet actions()}
          ...
          <div class="right">
            <button class="tool-btn" onclick={openImportDialog} title="Import contacts">Import</button>
          </div>
        {/snippet}
      </ListSearchBar>
