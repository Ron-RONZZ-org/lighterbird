...
from lighterbird.contacts.services.duplicate_detection import (
    _find_duplicate_groups,
    _merge_contacts_data,
)
from lighterbird.contacts.services.vcf_conversion import (
    _contact_to_vcard,
    _vcard_to_contact,
)
...
class ContactService(CRUDService):
    ...
    def import_vcf(self, path: str) -> int:
        ...
        for vcard in vobject.readComponents(content):
            contact = _vcard_to_contact(vcard)
    ...
    def export_vcf(self, uuid: str | None = None, path: str | None = None) -> str:
        ...
        for contact in contacts:
            if not contact:
                continue
            lines.append(_contact_to_vcard(contact))
    ...
    def find_duplicates(self) -> list[list[dict[str, Any]]]:
        ...
        return _find_duplicate_groups(all_contacts)
    ...
    def merge_duplicates(self, keep_uuid: str, remove_uuids: list[str]) -> dict[str, Any]:
        ...
        updates = _merge_contacts_data(keep, removals)
        self.update(keep_uuid, updates)
        for rem in removals:
            self.delete(rem["uuid"], soft=False)
