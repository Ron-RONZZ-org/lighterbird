...
Shared across ``/api/v1/chat`` and ``/api/v1/prompt-commands/execute``.
    ...
    for _round in range(max_rounds):
        ...
        for tc in result.tool_calls:
            ...
            if level is not None and level >= PermissionLevel.WRITE:
                write_batch.append({
                    "tokens": path.split("."),
                    "flags": flags,
                    "description": resolve_command_desc(path, get_handler_metadata_fn),
                })
                continue
        ...
        if write_batch:
            ...
            _pending_executions[session_id] = {
                ...
                "tools": tools,
                "name": name,
                "write_paths": {w["tokens"]: w for w in write_batch},
            }
