...
"""Maps dot-separated tool paths (e.g. ``"email.find"``) to their metadata dict.
...
def llm_tool(
    ...
    Args:
        name: Dot-separated tool path (e.g. ``"email.find"``).
...
def _validate_tool_name(name: str) -> None:
    ...
    if not name or not name.replace(".", "").isidentifier():
        raise ValueError(
            ...
            "Use dot-separated lowercase identifiers (e.g. 'email.find')."
        )
...
def get_llm_tools() -> list[dict[str, Any]]:
    ...
    Each tool's ``function.name`` uses underscores (e.g. ``"email_find"``)
    so that :func:`~lightercore.llm.tool_loop.tc_path` correctly converts
    it back to the dot-separated path (``"email.find"``).
...
def dispatch_llm_tool(path: str, flags: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        ...
        flags: Keyword arguments to pass to the handler.
